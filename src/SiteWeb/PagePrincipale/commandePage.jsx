...
// Import corrigé du chemin vers brandsApi
...
const CommandePage = () => {
  ...
  const [userInfo, setUserInfo] = useState({ clientType: 'particulier', nom: '', email: '', phone: '', address: '', postalCode: '', ville: '', additionalInfo: '' });
  ...
  const loadArticle = useCallback(async () => {
    ...
    try {
      ...
      if (data.manufacturer?.toLowerCase() !== brandName.toLowerCase()) throw new Error("La marque ne correspond pas.");
      setArticle(data);
    } catch (e) {
  ...
  useEffect(() => {
    if (brandName && article) {
      preloadKeysData(brandName).then(keys => {
  ...
  const basePrice = productDetails ?
    (isCleAPasse && productDetails.prixCleAPasse ? parseFloat(productDetails.prixCleAPasse) :
     (mode === 'postal' ? parseFloat(productDetails.prixSansCartePropriete) : parseFloat(productDetails.prix)))
    : 0;
  ...
  const handleOrder = async () => {
    if (!termsAccepted) { setSnackbarMessage('Veuillez accepter les CGV.'); setSnackbarSeverity('error'); setSnackbarOpen(true); return; }
    if (!validateForm()) { setSnackbarMessage('Champs obligatoires manquants.'); setSnackbarSeverity('error'); setSnackbarOpen(true); return; }
    setOrdering(true);
    try {
      ...
      const payload = { amount: Math.round(totalPrice*100), currency:'eur', description:`Paiement ${userInfo.nom}`, success_url:`https://www.cleservice.com/commande-success?numeroCommande=${numeroCommande}`, cancel_url:`https://www.cleservice.com/commande-cancel?numeroCommande=${numeroCommande}` };
      const payRes = await fetch('https://cl-back.onrender.com/stripe/create', { method:'POST', headers:{ 'Content-Type':'application/json'}, body:JSON.stringify(payload) });
  ...
  if (loadingArticle) return <Box sx={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center'}}><CircularProgress/></Box>;
  if (errorArticle) return <Box sx={{minHeight:'100vh',p:4,textAlign:'center'}}><ErrorIcon color="error" sx={{fontSize:48}}/><Typography variant="h6" color="error">{errorArticle}</Typography><Button onClick={loadArticle}>Réessayer</Button></Box>;
  ...
  return (
    <Box sx={{backgroundColor:'#f7f7f7',minHeight:'100vh',py:4}}>
      ...
      <Dialog open={openImageModal} onClose={handleCloseImageModal} maxWidth="md" fullWidth>
        <DialogContent sx={{p:0}}><img src={productDetails?.imageUrl} alt={productDetails?.nom} style={{width:'100%'}}/></DialogContent>
      </Dialog>
