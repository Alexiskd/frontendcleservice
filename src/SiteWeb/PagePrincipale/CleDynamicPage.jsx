...
function formatPrice(price) {
  // Vérifie si le prix est un nombre valide (en le forçant à être un nombre à virgule flottante)
  const parsedPrice = parseFloat(price);

  // Si le prix n'est pas un nombre valide, renvoyer "0,00"
  if (isNaN(parsedPrice)) {
    return '0,00';
  }

  // Si c'est un entier, ajoute ",00"
  if (Number.isInteger(parsedPrice)) {
    return parsedPrice.toFixed(2).replace('.', ','); // Ajoute ",00" si c'est un entier
  }

  // Pour les prix avec décimales, on garde la virgule
  ...
  useEffect(() => {
    if (/^\d+-/.test(brandFull)) {
      ...
      if (parts.length >= 3) {
        ...
        navigate(`/produit/${brand}/${encodeURIComponent(productName)}`); // Utilisation des backticks ici
      } else {
        navigate(`/produit/${encodeURIComponent(brandFull)}`); // Utilisation des backticks ici aussi
      }
    }
  }, [brandFull, navigate]);
