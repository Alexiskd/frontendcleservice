...
const CleDynamicPage = () => {
  ...
  // Combine brandFull et brandName, retire ".php"
  ...
  useEffect(() => {
    if (brandFull && /^\d+-/.test(brandFull)) return;
    if (!brandFull) return;
    fetch(`https://cl-back.onrender.com/brands/logo/${encodeURIComponent(brandFull)}`)
      .then((res) => {
        if (res.ok) return res.blob();
        throw new Error(`Logo non trouvé pour ${brandFull}`);
      })
      .then((blob) => {
